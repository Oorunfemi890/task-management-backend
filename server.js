...
app.get('/api/status', authenticate, async (req, res) => {
  try {
    ...
    const messageCount = await pool.query('SELECT COUNT(*) as count FROM project_messages WHERE is_archived = false');
...
server.listen(PORT, () => {
  ...
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    console.warn('⚠️  WARNING: Email configuration incomplete. Password reset and notifications will not work.');
  }
  
  if (!process.env.FRONTEND_URL) {
    console.warn('⚠️  WARNING: FRONTEND_URL not set. CORS may not work properly in production.');
  }
