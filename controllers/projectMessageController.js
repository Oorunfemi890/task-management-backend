...
const projectMessageController = {
      ...
      let query = `
        ...
        LEFT JOIN (
          SELECT 
            ...
            json_agg(
              jsonb_build_object('id', user_id, 'name', u2.name, 'avatar', u2.avatar)
            ) as users
    ...
    try {
      ...
      `,
        [
          ...
          `${currentUser.name}: ${message.substring(0, 100)}${
            message.length > 100 ? "..." : ""
          }`,
          JSON.stringify({
      ...
      try {
        ...
        `,
          [
            projectId,
            currentUser.id,
            message.trim() || file.originalname,
            messageType,
            attachmentUrl,
            file.originalname,
            file.size,
    ...
    try {
      ...
      if (messageCheck.rows.length === 0) {
        return res.status(403).json({
          error: "Access denied",
          message:
            "You can only delete your own messages or you must be a project admin",
        });
      }
      ...
      await pool.query("DELETE FROM message_reactions WHERE message_id = $1", [
        messageId,
      ]);
      await pool.query("DELETE FROM project_messages WHERE id = $1", [
        messageId,
      ]);
    ...
    try {
      ...
      if (req.io) {
        req.io
          ...
          .emit("project:message_reaction_updated", {
            projectId: parseInt(projectId),
            messageId: parseInt(messageId),
  ...
  archiveMessages: async (req, res) => {
    try {
      ...
      const archiveDate =
        beforeDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000); // 30 days ago
  ...
  getChatParticipants: async (req, res) => {
    try {
      ...
      LEFT JOIN project_chat_participants pcp 
        ON pcp.project_id = $1 AND pcp.user_id = u.id
